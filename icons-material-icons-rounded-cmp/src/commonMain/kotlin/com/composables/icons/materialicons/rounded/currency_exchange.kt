package com.composables.icons.materialicons.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.materialicons.MaterialIcons
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.group
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MaterialIcons.Rounded.Currency_exchange: ImageVector
    get() {
        if (_Currency_exchange != null) return _Currency_exchange!!
        
        _Currency_exchange = ImageVector.Builder(
            name = "currency_exchange",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 24f,
            viewportHeight = 24f
        ).apply {
            group {
                path(
                    fill = SolidColor(Color.Transparent)
                ) {
                    moveTo(0f, 0f)
                    horizontalLineTo(24f)
                    verticalLineTo(24f)
                    horizontalLineTo(0f)
                    verticalLineTo(0f)
                    close()
                }
            }
            group {
                path(
                    fill = SolidColor(Color(0xFF000000))
                ) {
                    moveTo(12f, 23f)
                    curveToRelative(5.7f, 0f, 10.39f, -4.34f, 10.95f, -9.9f)
                    curveToRelative(0.06f, -0.59f, -0.41f, -1.1f, -1f, -1.1f)
                    curveToRelative(-0.51f, 0f, -0.94f, 0.38f, -0.99f, 0.88f)
                    curveTo(20.52f, 17.44f, 16.67f, 21f, 12f, 21f)
                    curveToRelative(-3.12f, 0f, -5.87f, -1.59f, -7.48f, -4f)
                    lineTo(6f, 17f)
                    curveToRelative(0.55f, 0f, 1f, -0.45f, 1f, -1f)
                    reflectiveCurveToRelative(-0.45f, -1f, -1f, -1f)
                    horizontalLineTo(2f)
                    curveToRelative(-0.55f, 0f, -1f, 0.45f, -1f, 1f)
                    verticalLineToRelative(4f)
                    curveToRelative(0f, 0.55f, 0.45f, 1f, 1f, 1f)
                    curveToRelative(0.55f, 0f, 1f, -0.45f, 1f, -1f)
                    lineToRelative(0f, -1.67f)
                    curveTo(4.99f, 21.15f, 8.28f, 23f, 12f, 23f)
                    close()
                    moveTo(12f, 1f)
                    curveTo(6.3f, 1f, 1.61f, 5.34f, 1.05f, 10.9f)
                    curveTo(1f, 11.49f, 1.46f, 12f, 2.05f, 12f)
                    curveToRelative(0.51f, 0f, 0.94f, -0.38f, 0.99f, -0.88f)
                    curveTo(3.48f, 6.56f, 7.33f, 3f, 12f, 3f)
                    curveToRelative(3.12f, 0f, 5.87f, 1.59f, 7.48f, 4f)
                    lineTo(18f, 7f)
                    curveToRelative(-0.55f, 0f, -1f, 0.45f, -1f, 1f)
                    curveToRelative(0f, 0.55f, 0.45f, 1f, 1f, 1f)
                    horizontalLineToRelative(4f)
                    curveToRelative(0.55f, 0f, 1f, -0.45f, 1f, -1f)
                    verticalLineTo(4f)
                    curveToRelative(0f, -0.55f, -0.45f, -1f, -1f, -1f)
                    reflectiveCurveToRelative(-1f, 0.45f, -1f, 1f)
                    lineToRelative(0f, 1.67f)
                    curveTo(19.01f, 2.85f, 15.72f, 1f, 12f, 1f)
                    close()
                    moveTo(11.12f, 5.88f)
                    curveTo(11.12f, 5.39f, 11.52f, 5f, 12f, 5f)
                    reflectiveCurveToRelative(0.88f, 0.39f, 0.88f, 0.88f)
                    lineToRelative(0f, 0.37f)
                    curveToRelative(1.07f, 0.19f, 1.75f, 0.76f, 2.16f, 1.3f)
                    curveToRelative(0.34f, 0.44f, 0.16f, 1.08f, -0.36f, 1.3f)
                    curveTo(14.32f, 9f, 13.9f, 8.88f, 13.66f, 8.57f)
                    curveToRelative(-0.28f, -0.38f, -0.78f, -0.77f, -1.6f, -0.77f)
                    curveToRelative(-0.7f, 0f, -1.81f, 0.37f, -1.81f, 1.39f)
                    curveToRelative(0f, 0.95f, 0.86f, 1.31f, 2.64f, 1.9f)
                    curveToRelative(2.4f, 0.83f, 3.01f, 2.05f, 3.01f, 3.45f)
                    curveToRelative(0f, 2.62f, -2.5f, 3.13f, -3.02f, 3.22f)
                    lineToRelative(0f, 0.37f)
                    curveToRelative(0f, 0.48f, -0.39f, 0.88f, -0.88f, 0.88f)
                    reflectiveCurveToRelative(-0.88f, -0.39f, -0.88f, -0.88f)
                    lineToRelative(0f, -0.42f)
                    curveToRelative(-0.63f, -0.15f, -1.93f, -0.61f, -2.69f, -2.1f)
                    curveToRelative(-0.23f, -0.44f, 0.03f, -1.02f, 0.49f, -1.2f)
                    curveToRelative(0.41f, -0.16f, 0.9f, -0.01f, 1.11f, 0.38f)
                    curveToRelative(0.32f, 0.61f, 0.95f, 1.37f, 2.12f, 1.37f)
                    curveToRelative(0.93f, 0f, 1.98f, -0.48f, 1.98f, -1.61f)
                    curveToRelative(0f, -0.96f, -0.7f, -1.46f, -2.28f, -2.03f)
                    curveToRelative(-1.1f, -0.39f, -3.35f, -1.03f, -3.35f, -3.31f)
                    curveToRelative(0f, -0.1f, 0.01f, -2.4f, 2.62f, -2.96f)
                    lineTo(11.12f, 5.88f)
                    close()
                }
            }
        }.build()
        
        return _Currency_exchange!!
    }

private var _Currency_exchange: ImageVector? = null

