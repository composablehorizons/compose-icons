package com.composables.icons.materialsymbols.rounded

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.materialsymbols.MaterialSymbols
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MaterialSymbols.Rounded.Zone_person_idle: ImageVector
    get() {
        if (_Zone_person_idle != null) return _Zone_person_idle!!
        
        _Zone_person_idle = ImageVector.Builder(
            name = "zone_person_idle",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 960f,
            viewportHeight = 960f
        ).apply {
            path(
                fill = SolidColor(Color(0xFF000000))
            ) {
                moveTo(791f, 853f)
                lineTo(528f, 590f)
                lineToRelative(-16f, 78f)
                horizontalLineTo(352f)
                lineToRelative(40f, -204f)
                lineToRelative(-72f, 28f)
                verticalLineToRelative(96f)
                quadToRelative(0f, 17f, -11.5f, 28.5f)
                reflectiveQuadTo(280f, 628f)
                quadToRelative(-17f, 0f, -28.5f, -11.5f)
                reflectiveQuadTo(240f, 588f)
                verticalLineToRelative(-122f)
                quadToRelative(0f, -12f, 6.5f, -21.5f)
                reflectiveQuadTo(264f, 430f)
                lineToRelative(72f, -31f)
                lineTo(55f, 118f)
                quadToRelative(-12f, -12f, -11.5f, -28.5f)
                reflectiveQuadTo(56f, 61f)
                quadToRelative(12f, -12f, 28.5f, -12f)
                reflectiveQuadToRelative(28.5f, 12f)
                lineToRelative(735f, 736f)
                quadToRelative(12f, 12f, 12f, 28f)
                reflectiveQuadToRelative(-12f, 28f)
                quadToRelative(-12f, 12f, -28.5f, 12f)
                reflectiveQuadTo(791f, 853f)
                close()
                moveToRelative(-631f, -25f)
                quadToRelative(-33f, 0f, -56.5f, -23.5f)
                reflectiveQuadTo(80f, 748f)
                verticalLineToRelative(-80f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(120f, 628f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(160f, 668f)
                verticalLineToRelative(80f)
                horizontalLineToRelative(80f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(280f, 788f)
                quadToRelative(0f, 17f, -11.5f, 28.5f)
                reflectiveQuadTo(240f, 828f)
                horizontalLineToRelative(-80f)
                close()
                moveToRelative(640f, -640f)
                verticalLineToRelative(-80f)
                horizontalLineToRelative(-80f)
                quadToRelative(-17f, 0f, -28.5f, -11.5f)
                reflectiveQuadTo(680f, 68f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(720f, 28f)
                horizontalLineToRelative(80f)
                quadToRelative(33f, 0f, 56.5f, 23.5f)
                reflectiveQuadTo(880f, 108f)
                verticalLineToRelative(80f)
                quadToRelative(0f, 17f, -11.5f, 28.5f)
                reflectiveQuadTo(840f, 228f)
                quadToRelative(-17f, 0f, -28.5f, -11.5f)
                reflectiveQuadTo(800f, 188f)
                close()
                moveTo(80f, 228f)
                verticalLineToRelative(-100f)
                quadToRelative(0f, -17f, 6.5f, -31.5f)
                reflectiveQuadTo(103f, 71f)
                lineToRelative(57f, 58f)
                verticalLineToRelative(99f)
                horizontalLineTo(80f)
                close()
                moveToRelative(640f, 600f)
                quadToRelative(-17f, 0f, -28.5f, -11.5f)
                reflectiveQuadTo(680f, 788f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(720f, 748f)
                horizontalLineToRelative(69f)
                lineToRelative(57f, 56f)
                quadToRelative(-10f, 11f, -24.5f, 17.5f)
                reflectiveQuadTo(790f, 828f)
                horizontalLineToRelative(-70f)
                close()
                moveTo(273f, 108f)
                lineToRelative(-80f, -80f)
                horizontalLineToRelative(87f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(320f, 68f)
                quadToRelative(0f, 17f, -11.5f, 28.5f)
                reflectiveQuadTo(280f, 108f)
                horizontalLineToRelative(-7f)
                close()
                moveToRelative(607f, 607f)
                lineToRelative(-80f, -80f)
                verticalLineToRelative(-7f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(840f, 588f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(880f, 628f)
                verticalLineToRelative(87f)
                close()
                moveTo(540f, 328f)
                quadToRelative(-33f, 0f, -56.5f, -23.5f)
                reflectiveQuadTo(460f, 248f)
                quadToRelative(0f, -33f, 23.5f, -56.5f)
                reflectiveQuadTo(540f, 168f)
                quadToRelative(33f, 0f, 56.5f, 23.5f)
                reflectiveQuadTo(620f, 248f)
                quadToRelative(0f, 33f, -23.5f, 56.5f)
                reflectiveQuadTo(540f, 328f)
                close()
            }
        }.build()
        
        return _Zone_person_idle!!
    }

private var _Zone_person_idle: ImageVector? = null

