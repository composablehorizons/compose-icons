package com.composables.icons.fontawesome.solid

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.fontawesome.FontAwesome
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val FontAwesome.Solid.Podcast: ImageVector
    get() {
        if (_Podcast != null) return _Podcast!!
        
        _Podcast = ImageVector.Builder(
            name = "podcast",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 448f,
            viewportHeight = 512f
        ).apply {
            path(
                fill = SolidColor(Color(0xFF000000))
            ) {
                moveTo(267.429f, 488.563f)
                curveTo(262.286f, 507.573f, 242.858f, 512f, 224f, 512f)
                curveToRelative(-18.857f, 0f, -38.286f, -4.427f, -43.428f, -23.437f)
                curveTo(172.927f, 460.134f, 160f, 388.898f, 160f, 355.75f)
                curveToRelative(0f, -35.156f, 31.142f, -43.75f, 64f, -43.75f)
                reflectiveCurveToRelative(64f, 8.594f, 64f, 43.75f)
                curveToRelative(0f, 32.949f, -12.871f, 104.179f, -20.571f, 132.813f)
                close()
                moveTo(156.867f, 288.554f)
                curveToRelative(-18.693f, -18.308f, -29.958f, -44.173f, -28.784f, -72.599f)
                curveToRelative(2.054f, -49.724f, 42.395f, -89.956f, 92.124f, -91.881f)
                curveTo(274.862f, 121.958f, 320f, 165.807f, 320f, 220f)
                curveToRelative(0f, 26.827f, -11.064f, 51.116f, -28.866f, 68.552f)
                curveToRelative(-2.675f, 2.62f, -2.401f, 6.986f, 0.628f, 9.187f)
                curveToRelative(9.312f, 6.765f, 16.46f, 15.343f, 21.234f, 25.363f)
                curveToRelative(1.741f, 3.654f, 6.497f, 4.66f, 9.449f, 1.891f)
                curveToRelative(28.826f, -27.043f, 46.553f, -65.783f, 45.511f, -108.565f)
                curveToRelative(-1.855f, -76.206f, -63.595f, -138.208f, -139.793f, -140.369f)
                curveTo(146.869f, 73.753f, 80f, 139.215f, 80f, 220f)
                curveToRelative(0f, 41.361f, 17.532f, 78.7f, 45.55f, 104.989f)
                curveToRelative(2.953f, 2.771f, 7.711f, 1.77f, 9.453f, -1.887f)
                curveToRelative(4.774f, -10.021f, 11.923f, -18.598f, 21.235f, -25.363f)
                curveToRelative(3.029f, -2.2f, 3.304f, -6.566f, 0.629f, -9.185f)
                close()
                moveTo(224f, 0f)
                curveTo(100.204f, 0f, 0f, 100.185f, 0f, 224f)
                curveToRelative(0f, 89.992f, 52.602f, 165.647f, 125.739f, 201.408f)
                curveToRelative(4.333f, 2.118f, 9.267f, -1.544f, 8.535f, -6.31f)
                curveToRelative(-2.382f, -15.512f, -4.342f, -30.946f, -5.406f, -44.339f)
                curveToRelative(-0.146f, -1.836f, -1.149f, -3.486f, -2.678f, -4.512f)
                curveToRelative(-47.4f, -31.806f, -78.564f, -86.016f, -78.187f, -147.347f)
                curveToRelative(0.592f, -96.237f, 79.29f, -174.648f, 175.529f, -174.899f)
                curveTo(320.793f, 47.747f, 400f, 126.797f, 400f, 224f)
                curveToRelative(0f, 61.932f, -32.158f, 116.49f, -80.65f, 147.867f)
                curveToRelative(-0.999f, 14.037f, -3.069f, 30.588f, -5.624f, 47.23f)
                curveToRelative(-0.732f, 4.767f, 4.203f, 8.429f, 8.535f, 6.31f)
                curveTo(395.227f, 389.727f, 448f, 314.187f, 448f, 224f)
                curveTo(448f, 100.205f, 347.815f, 0f, 224f, 0f)
                close()
                moveToRelative(0f, 160f)
                curveToRelative(-35.346f, 0f, -64f, 28.654f, -64f, 64f)
                reflectiveCurveToRelative(28.654f, 64f, 64f, 64f)
                reflectiveCurveToRelative(64f, -28.654f, 64f, -64f)
                reflectiveCurveToRelative(-28.654f, -64f, -64f, -64f)
                close()
            }
        }.build()
        
        return _Podcast!!
    }

private var _Podcast: ImageVector? = null

