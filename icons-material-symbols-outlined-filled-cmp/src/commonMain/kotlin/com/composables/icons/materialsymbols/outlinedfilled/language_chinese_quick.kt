package com.composables.icons.materialsymbols.outlinedfilled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.materialsymbols.MaterialSymbols
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MaterialSymbols.OutlinedFilled.Language_chinese_quick: ImageVector
    get() {
        if (_Language_chinese_quick != null) return _Language_chinese_quick!!
        
        _Language_chinese_quick = ImageVector.Builder(
            name = "language_chinese_quick",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 960f,
            viewportHeight = 960f
        ).apply {
            path(
                fill = SolidColor(Color(0xFF000000))
            ) {
                moveTo(98f, 702f)
                lineToRelative(-25f, -43f)
                quadToRelative(21f, -11f, 39f, -25.5f)
                reflectiveQuadToRelative(36f, -29.5f)
                verticalLineToRelative(-115f)
                horizontalLineTo(81f)
                verticalLineToRelative(-44f)
                horizontalLineToRelative(113f)
                verticalLineToRelative(156f)
                quadToRelative(20f, 25f, 50f, 38f)
                reflectiveQuadToRelative(62f, 13f)
                quadToRelative(56f, 0f, 111.5f, -1f)
                reflectiveQuadToRelative(111.5f, -4f)
                quadToRelative(-5f, 11f, -9f, 23f)
                reflectiveQuadToRelative(-7f, 24f)
                quadToRelative(-52f, 2f, -103.5f, 1f)
                reflectiveQuadTo(306f, 694f)
                quadToRelative(-38f, 0f, -74f, -13f)
                reflectiveQuadToRelative(-62f, -41f)
                quadToRelative(-17f, 17f, -35f, 32.5f)
                reflectiveQuadTo(98f, 702f)
                close()
                moveToRelative(238f, -58f)
                verticalLineToRelative(-106f)
                quadToRelative(-22f, 29f, -50f, 51f)
                reflectiveQuadToRelative(-58f, 42f)
                quadToRelative(-7f, -9f, -14.5f, -17.5f)
                reflectiveQuadTo(197f, 597f)
                quadToRelative(35f, -15f, 64.5f, -38f)
                reflectiveQuadToRelative(52.5f, -52f)
                horizontalLineToRelative(-84f)
                verticalLineToRelative(-129f)
                horizontalLineToRelative(107f)
                verticalLineToRelative(-30f)
                horizontalLineTo(210f)
                verticalLineToRelative(-41f)
                horizontalLineToRelative(127f)
                verticalLineToRelative(-48f)
                horizontalLineToRelative(44f)
                verticalLineToRelative(48f)
                horizontalLineToRelative(136f)
                verticalLineToRelative(41f)
                horizontalLineTo(381f)
                verticalLineToRelative(30f)
                horizontalLineToRelative(113f)
                verticalLineToRelative(128f)
                horizontalLineToRelative(-91f)
                quadToRelative(25f, 29f, 55f, 51f)
                reflectiveQuadToRelative(65f, 37f)
                quadToRelative(-10f, 8f, -18.5f, 16.5f)
                reflectiveQuadTo(491f, 630f)
                quadToRelative(-32f, -17f, -59.5f, -40.5f)
                reflectiveQuadTo(381f, 539f)
                verticalLineToRelative(105f)
                horizontalLineToRelative(-45f)
                close()
                moveToRelative(0f, -227f)
                horizontalLineToRelative(-61f)
                verticalLineToRelative(51f)
                horizontalLineToRelative(61f)
                verticalLineToRelative(-51f)
                close()
                moveToRelative(114f, 0f)
                horizontalLineToRelative(-70f)
                verticalLineToRelative(51f)
                horizontalLineToRelative(70f)
                verticalLineToRelative(-51f)
                close()
                moveToRelative(-278f, -37f)
                quadToRelative(-17f, -26f, -39.5f, -46.5f)
                reflectiveQuadTo(83f, 298f)
                lineToRelative(34f, -29f)
                quadToRelative(26f, 17f, 47.5f, 38f)
                reflectiveQuadToRelative(43.5f, 43f)
                lineToRelative(-36f, 30f)
                close()
                moveToRelative(633f, 196f)
                verticalLineToRelative(-106f)
                quadToRelative(-23f, 3f, -42.5f, 7f)
                reflectiveQuadToRelative(-36.5f, 9f)
                quadToRelative(-45f, 14f, -67.5f, 35f)
                reflectiveQuadTo(636f, 570f)
                quadToRelative(0f, 26f, 18f, 41f)
                reflectiveQuadToRelative(48f, 15f)
                quadToRelative(28f, 0f, 54.5f, -13f)
                reflectiveQuadToRelative(48.5f, -37f)
                close()
                moveToRelative(13f, 109f)
                lineToRelative(-6f, -45f)
                quadToRelative(-28f, 25f, -61.5f, 39.5f)
                reflectiveQuadTo(679f, 694f)
                quadToRelative(-57f, 0f, -90f, -32.5f)
                reflectiveQuadTo(556f, 573f)
                quadToRelative(0f, -52f, 33f, -85f)
                reflectiveQuadToRelative(106f, -53f)
                quadToRelative(23f, -6f, 50.5f, -11f)
                reflectiveQuadToRelative(59.5f, -9f)
                quadToRelative(-2f, -47f, -22f, -68.5f)
                reflectiveQuadTo(721f, 325f)
                quadToRelative(-26f, 0f, -51.5f, 9.5f)
                reflectiveQuadTo(604f, 368f)
                lineToRelative(-32f, -56f)
                quadToRelative(33f, -25f, 77.5f, -40.5f)
                reflectiveQuadTo(740f, 256f)
                quadToRelative(71f, 0f, 108f, 44f)
                reflectiveQuadToRelative(37f, 128f)
                verticalLineToRelative(257f)
                horizontalLineToRelative(-67f)
                close()
            }
        }.build()
        
        return _Language_chinese_quick!!
    }

private var _Language_chinese_quick: ImageVector? = null

