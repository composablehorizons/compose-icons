package com.composables.icons.materialsymbols.outlinedfilled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.materialsymbols.MaterialSymbols
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MaterialSymbols.OutlinedFilled.Language_chinese_pinyin: ImageVector
    get() {
        if (_Language_chinese_pinyin != null) return _Language_chinese_pinyin!!
        
        _Language_chinese_pinyin = ImageVector.Builder(
            name = "language_chinese_pinyin",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 960f,
            viewportHeight = 960f
        ).apply {
            path(
                fill = SolidColor(Color(0xFF000000))
            ) {
                moveTo(340f, 492f)
                horizontalLineToRelative(56f)
                verticalLineToRelative(-82f)
                horizontalLineToRelative(-56f)
                verticalLineToRelative(82f)
                close()
                moveTo(101f, 700f)
                lineToRelative(-17f, -45f)
                horizontalLineToRelative(39f)
                quadToRelative(5f, 0f, 9.5f, -3.5f)
                reflectiveQuadToRelative(4.5f, -8.5f)
                verticalLineToRelative(-101f)
                lineToRelative(-52f, 17f)
                lineToRelative(-11f, -44f)
                lineToRelative(63f, -19f)
                verticalLineToRelative(-90f)
                horizontalLineTo(81f)
                verticalLineToRelative(-42f)
                horizontalLineToRelative(56f)
                verticalLineToRelative(-92f)
                horizontalLineToRelative(44f)
                verticalLineToRelative(92f)
                horizontalLineToRelative(45f)
                verticalLineToRelative(42f)
                horizontalLineToRelative(-46f)
                verticalLineToRelative(77f)
                lineToRelative(40f, -13f)
                lineToRelative(6f, 42f)
                lineToRelative(-46f, 16f)
                verticalLineToRelative(125f)
                quadToRelative(0f, 18f, -10.5f, 32.5f)
                reflectiveQuadTo(142f, 700f)
                horizontalLineToRelative(-41f)
                close()
                moveToRelative(139f, 6f)
                lineToRelative(-29f, -32f)
                quadToRelative(35f, -23f, 57.5f, -59f)
                reflectiveQuadToRelative(23.5f, -79f)
                horizontalLineToRelative(-68f)
                verticalLineToRelative(-45f)
                horizontalLineToRelative(72f)
                verticalLineToRelative(-81f)
                horizontalLineToRelative(-54f)
                verticalLineToRelative(-41f)
                horizontalLineToRelative(253f)
                verticalLineToRelative(44f)
                horizontalLineToRelative(-57f)
                verticalLineToRelative(83f)
                horizontalLineToRelative(70f)
                lineToRelative(-2f, 40f)
                horizontalLineToRelative(-68f)
                verticalLineToRelative(165f)
                horizontalLineToRelative(-44f)
                verticalLineToRelative(-165f)
                horizontalLineToRelative(-58f)
                quadToRelative(-2f, 52f, -27.5f, 96.5f)
                reflectiveQuadTo(240f, 706f)
                close()
                moveToRelative(193f, -327f)
                lineToRelative(-41f, -10f)
                lineToRelative(22f, -48f)
                quadToRelative(11f, -24f, 19f, -49f)
                lineToRelative(45f, 16f)
                quadToRelative(-10f, 23f, -21.5f, 45.5f)
                reflectiveQuadTo(433f, 379f)
                close()
                moveToRelative(-121f, -2f)
                quadToRelative(-10f, -23f, -21f, -45f)
                reflectiveQuadToRelative(-25f, -42f)
                lineToRelative(40f, -17f)
                quadToRelative(14f, 20f, 25f, 42f)
                reflectiveQuadToRelative(21f, 45f)
                lineToRelative(-40f, 17f)
                close()
                moveToRelative(390f, 249f)
                quadToRelative(28f, 0f, 54.5f, -13f)
                reflectiveQuadToRelative(48.5f, -37f)
                verticalLineToRelative(-106f)
                quadToRelative(-23f, 3f, -42.5f, 7f)
                reflectiveQuadToRelative(-36.5f, 9f)
                quadToRelative(-45f, 14f, -67.5f, 35f)
                reflectiveQuadTo(636f, 570f)
                quadToRelative(0f, 26f, 18f, 41f)
                reflectiveQuadToRelative(48f, 15f)
                close()
                moveToRelative(-23f, 68f)
                quadToRelative(-57f, 0f, -90f, -32.5f)
                reflectiveQuadTo(556f, 573f)
                quadToRelative(0f, -52f, 33f, -85f)
                reflectiveQuadToRelative(106f, -53f)
                quadToRelative(23f, -6f, 50.5f, -11f)
                reflectiveQuadToRelative(59.5f, -9f)
                quadToRelative(-2f, -47f, -22f, -68.5f)
                reflectiveQuadTo(721f, 325f)
                quadToRelative(-26f, 0f, -51.5f, 9.5f)
                reflectiveQuadTo(604f, 368f)
                lineToRelative(-32f, -56f)
                quadToRelative(33f, -25f, 77.5f, -40.5f)
                reflectiveQuadTo(740f, 256f)
                quadToRelative(71f, 0f, 108f, 44f)
                reflectiveQuadToRelative(37f, 128f)
                verticalLineToRelative(257f)
                horizontalLineToRelative(-67f)
                lineToRelative(-6f, -45f)
                quadToRelative(-28f, 25f, -61.5f, 39.5f)
                reflectiveQuadTo(679f, 694f)
                close()
            }
        }.build()
        
        return _Language_chinese_pinyin!!
    }

private var _Language_chinese_pinyin: ImageVector? = null

