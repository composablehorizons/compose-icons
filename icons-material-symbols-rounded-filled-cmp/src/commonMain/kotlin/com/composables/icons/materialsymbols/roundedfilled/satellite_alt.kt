package com.composables.icons.materialsymbols.roundedfilled

import androidx.compose.ui.graphics.Color
import androidx.compose.ui.graphics.SolidColor
import com.composables.icons.materialsymbols.MaterialSymbols
import androidx.compose.ui.graphics.vector.ImageVector
import androidx.compose.ui.graphics.vector.path
import androidx.compose.ui.unit.dp

val MaterialSymbols.RoundedFilled.Satellite_alt: ImageVector
    get() {
        if (_Satellite_alt != null) return _Satellite_alt!!
        
        _Satellite_alt = ImageVector.Builder(
            name = "satellite_alt",
            defaultWidth = 24.dp,
            defaultHeight = 24.dp,
            viewportWidth = 960f,
            viewportHeight = 960f
        ).apply {
            path(
                fill = SolidColor(Color(0xFF000000))
            ) {
                moveTo(561f, 928f)
                quadToRelative(-17f, 0f, -29f, -11.5f)
                reflectiveQuadTo(520f, 888f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(560f, 848f)
                quadToRelative(117f, 0f, 198.5f, -81.5f)
                reflectiveQuadTo(840f, 568f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(880f, 528f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(920f, 568f)
                quadToRelative(0f, 74f, -28.5f, 139.5f)
                reflectiveQuadToRelative(-77f, 114.5f)
                quadToRelative(-48.5f, 49f, -114f, 77.5f)
                reflectiveQuadTo(561f, 928f)
                close()
                moveToRelative(0f, -160f)
                quadToRelative(-17f, 0f, -29f, -11.5f)
                reflectiveQuadTo(520f, 728f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(560f, 688f)
                quadToRelative(50f, 0f, 85f, -35f)
                reflectiveQuadToRelative(35f, -85f)
                quadToRelative(0f, -17f, 11.5f, -28.5f)
                reflectiveQuadTo(720f, 528f)
                quadToRelative(17f, 0f, 28.5f, 11.5f)
                reflectiveQuadTo(760f, 568f)
                quadToRelative(-1f, 83f, -58.5f, 141f)
                reflectiveQuadTo(561f, 768f)
                close()
                moveTo(222f, 903f)
                quadToRelative(-15f, 0f, -30f, -6f)
                reflectiveQuadToRelative(-27f, -17f)
                lineTo(23f, 738f)
                quadToRelative(-11f, -12f, -17f, -27f)
                reflectiveQuadToRelative(-6f, -30f)
                quadToRelative(0f, -16f, 6f, -30.5f)
                reflectiveQuadTo(23f, 625f)
                lineToRelative(127f, -127f)
                quadToRelative(23f, -23f, 57f, -23.5f)
                reflectiveQuadToRelative(57f, 22.5f)
                lineToRelative(50f, 50f)
                lineToRelative(28f, -28f)
                lineToRelative(-50f, -50f)
                quadToRelative(-23f, -23f, -23f, -56f)
                reflectiveQuadToRelative(23f, -56f)
                lineToRelative(57f, -57f)
                quadToRelative(23f, -23f, 56.5f, -23f)
                reflectiveQuadToRelative(56.5f, 23f)
                lineToRelative(50f, 50f)
                lineToRelative(28f, -28f)
                lineToRelative(-50f, -50f)
                quadToRelative(-23f, -23f, -23f, -56.5f)
                reflectiveQuadToRelative(23f, -56.5f)
                lineToRelative(127f, -127f)
                quadToRelative(12f, -12f, 27f, -18f)
                reflectiveQuadToRelative(30f, -6f)
                quadToRelative(15f, 0f, 29.5f, 6f)
                reflectiveQuadToRelative(26.5f, 18f)
                lineToRelative(142f, 142f)
                quadToRelative(12f, 11f, 17.5f, 25.5f)
                reflectiveQuadTo(895f, 230f)
                quadToRelative(0f, 15f, -5.5f, 30f)
                reflectiveQuadTo(872f, 287f)
                lineTo(745f, 414f)
                quadToRelative(-23f, 23f, -56.5f, 23f)
                reflectiveQuadTo(632f, 414f)
                lineToRelative(-50f, -50f)
                lineToRelative(-28f, 28f)
                lineToRelative(50f, 50f)
                quadToRelative(23f, 23f, 22.5f, 56.5f)
                reflectiveQuadTo(603f, 555f)
                lineToRelative(-56f, 56f)
                quadToRelative(-23f, 23f, -56.5f, 23f)
                reflectiveQuadTo(434f, 611f)
                lineToRelative(-50f, -50f)
                lineToRelative(-28f, 28f)
                lineToRelative(50f, 50f)
                quadToRelative(23f, 23f, 22.5f, 57f)
                reflectiveQuadTo(405f, 753f)
                lineTo(278f, 880f)
                quadToRelative(-11f, 11f, -25.5f, 17f)
                reflectiveQuadTo(222f, 903f)
                close()
                moveToRelative(0f, -79f)
                lineToRelative(42f, -42f)
                lineToRelative(-142f, -142f)
                lineToRelative(-42f, 42f)
                lineToRelative(142f, 142f)
                close()
                moveToRelative(85f, -85f)
                lineToRelative(42f, -42f)
                lineToRelative(-142f, -142f)
                lineToRelative(-42f, 42f)
                lineToRelative(142f, 142f)
                close()
                moveToRelative(382f, -382f)
                lineToRelative(42f, -42f)
                lineToRelative(-142f, -142f)
                lineToRelative(-42f, 42f)
                lineToRelative(142f, 142f)
                close()
                moveToRelative(85f, -85f)
                lineToRelative(42f, -42f)
                lineToRelative(-142f, -142f)
                lineToRelative(-42f, 42f)
                lineToRelative(142f, 142f)
                close()
            }
        }.build()
        
        return _Satellite_alt!!
    }

private var _Satellite_alt: ImageVector? = null

